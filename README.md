  ...
  metricstransform:
    transforms:
      - include: traces.span.metrics.calls
        action: update
        new_name: curtis.rad.service.calls
      - include: traces.span.metrics.duration
        action: update
        new_name: curtis.rad.service.latency
  batch:
    metadata_keys:
      - X-SF-Token
...
service:
  ...
  pipelines:
    ...
    metrics:
      receivers: [spanmetrics,hostmetrics, otlp, signalfx]
      processors: [metricstransform,memory_limiter, batch, resourcedetection]
      exporters: [signalfx]
      # Use instead when sending to gateway
